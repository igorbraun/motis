...
void build_toy_scenario() {
  ...
  cap_ILP_connection c2{
      2, std::vector<cap_ILP_edge*>{&e3, &ew_3_4, &e4, &ew_4_5, &e5}};
  ...
  cap_ILP_connection c3{
      3, std::vector<cap_ILP_edge*>{&e6, &ew_6_7, &e7, &ew_7_8, &e8}};
  ...
  for (auto i = 0u; i < psg_groups.size(); ++i) {
    std::cout << "pg " << psg_groups[i].id_ << " should use "
              << psg_groups[i].alternatives_[psg_assignment[i]]->id_ << std::endl;
  }
}
